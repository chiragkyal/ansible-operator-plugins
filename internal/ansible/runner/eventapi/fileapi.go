...
func (f *FileEventReceiver) watchJobEvents() {
	...
	for {
		...
		// 	f.processExistingFiles(jobEventsDir)
		case <-f.ctx.Done():
			// When context is cancelled, do a final scan for any remaining files
			f.logger.V(1).Info("Context cancelled, doing final scan for events")
			// f.processExistingFiles(jobEventsDir)
			return
		}
	}
...
// 	if err != nil {
// 		f.logger.Info("Could not read job_events directory", "dir", jobEventsDir, "error", err)
// 		return
// 	}

// 	f.logger.Info("Scanning for existing event files", "dir", jobEventsDir, "fileCount", len(files))

// 	// Sort files by name to process in order (ansible-runner prefixes with sequence number)
// 	var jsonFiles []string
// 	for _, file := range files {
// 		if !file.IsDir() && filepath.Ext(file.Name()) == ".json" {
// 			fullPath := filepath.Join(jobEventsDir, file.Name())

// 			f.processMutex.Lock()
// 			alreadyProcessed := f.processedFiles[fullPath]
// 			f.processMutex.Unlock()
...
// 	f.logger.Info("Found event files to process", "count", len(jsonFiles))
...
// 		f.logger.V(2).Info("Processing event file", "file", filePath)
