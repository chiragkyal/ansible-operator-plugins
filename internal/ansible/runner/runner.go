...
func (r *runner) Run(ident string, u *unstructured.Unstructured, kubeconfig string) (RunResult, error) {
	...
	go func() {
		...
		if _, isFileReceiver := receiver.(*fileReceiverWrapper); isFileReceiver {
			logger.Info("Waiting for file-based event receiver to process final events")
			...
			if entries, err := os.ReadDir(artifactsDir); err == nil {
				logger.Info("Artifacts directory contents", "dir", artifactsDir, "entries", len(entries))
				for _, entry := range entries {
					logger.Info("Artifact entry", "name", entry.Name(), "isDir", entry.IsDir())
				}
			} else {
			...
			if entries, err := os.ReadDir(jobEventsDir); err == nil {
				logger.Info("Job events directory contents", "dir", jobEventsDir, "entries", len(entries))
				for _, entry := range entries {
					logger.Info("Job event file", "name", entry.Name())
				}
			} else {
